export const strangeTendrils = {
  frag: `//By Cale Bradbury, 2015
    ...
    vec2 map( in vec3 pos )
    {
       ...
       pos.xy = kale(pos.xy,pi/6.,pi*1.4+cos(uTime)*.1,0.);
        pos.xz = kale(pos.xz,pi/6.,pi*1.4+sin(uTime)*.1,0.);
        
        float size = .5;
        float r = f(pos,size,1.);  
        
        for(float i = 0.; i<LOOPS;i++){
            pos+=vec3(size*(sin(uTime)*.5+1.5),0.,0.);
            size*=(cos(uTime*2.)*.3+.5);
            r = f(pos,size,r);
        }
        return vec2(r,1.0);
    }
