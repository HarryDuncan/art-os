export const glossyBlobs = {
  frag: `//CC0 1.0 Universal https://creativecommons.org/publicdomain/zero/1.0/
    ...
    float scene(vec3 p) {
      float rad = 3.+p.z+sin(p.y/2.+uTime)+cos(p.x/3.+uTime*0.9);
      float dist = 10000.;
      for (int i = 0; i < 4; i++) {
        vec3 rot = hash33(vec3(float(i+1), cos(float(i)), sin(float(i))));
        float d = comp(p, rot, uTime/2.*(float(i+1)));
        dist = smin(dist, d, 1.);
      }
      return mix(dist, rad, mix(0.3,0.8+sin(uTime)*0.2, 0.1));
    }
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord )
    {
      ...
      vec3 col = vec3(1);
      vec3 p; vec3 n;
      vec3 init = vec3(-4,0,sin(uTime*0.37)*1.4);
      vec2 m = uPosition.xy/uResolution.xy;
      if (uPosition.z <= 0.) {
        m = vec2(0.4);
      }
      ...
      float fog = pow(smoothstep(drawdist*.5,drawdist,distance(p,origin)),sin(uTime) + 1.0);
    
      fragColor.xyz = mix(col*0.8+0.02, vec3(0.1),fog);
