export const lsd = {
  frag: `
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord )
    {
        ...
        vlsd = rotate3d(vlsd, vec3(1.,1.,0.), uTime);
        vlsd = rotate3d(vlsd, vec3(1.,1.,0.), uTime);
        vlsd = rotate3d(vlsd, vec3(1.,1.,0.), uTime);
        
        vec2 
            v0 = .75 * sincos(.3457 * uTime + .3423) - simplex2d(uv * .917),
            v1 = .75 * sincos(.7435 * uTime + .4565) - simplex2d(uv * .521), 
            v2 = .75 * sincos(.5345 * uTime + .3434) - simplex2d(uv * .759);
        ...
        color *= 1.- .25* vec3(
            varazslat(uv *.25, uTime + .5),
            varazslat(uv * .7, uTime + .2),
            varazslat(uv * .4, uTime + .7)
        );
