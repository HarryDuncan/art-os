...
export const vanishTransform = (
  ...
  const { declareInTransform } = vanishParameters;
  ...
  const transformation = `
        ...
        float percent =  mod(uTime * 0.5, 2.0);
        float target = percent - 1.35;
        ...
        vec3 col = vec3(simplePerlinNoise(vec3(1.0 - newPosition.z * uNumberOfRings)) * 50.0) * (0.01 - distortion) * ${fragName}.xyz * brightness;
        ${fragName}  = vec4(${fragName}.xyz * clamp(percent, 0.0, 0.5) + clamp(col, vec3(0.0), vec3(1.0)), 1.0);
